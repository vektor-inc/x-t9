...
[ Spec Change ] Changed the Sass breakpoint variables to a max-value-only set ( $xs / $sm / $md / $lg / $xl ) and rewrote the related media queries using range syntax ( width <= / > ) instead of min-width / max-width, aligning boundary handling with the WordPress standard
